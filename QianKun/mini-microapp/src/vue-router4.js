...
var VueRouter = (function (exports, vue) {
  ...
  function tokensToParser(segments, extraOptions) {
    ...
    if (options.strict && options.end) {
      ...
      score[i][
        score[i].length - 1
      ] += 0.7000000000000001 /* PathScore.BonusStrict */;
    }
  ...
  function compareScoreArray(a, b) {
    ...
    if (a.length < b.length) {
      return a.length === 1 &&
        a[0] === 40 /* PathScore.Static */ + 40 /* PathScore.Segment */
        ? -1
        : 1;
    } else if (a.length > b.length) {
      return b.length === 1 &&
        b[0] === 40 /* PathScore.Static */ + 40 /* PathScore.Segment */
        ? 1
        : -1;
    }
    return 0;
    ...
    function navigate(to, from) {
      ...
      return (
        runGuardQueue(guards)
          ...
          .catch((err) =>
            isNavigationFailure(err, 8 /* ErrorTypes.NAVIGATION_CANCELLED */)
              ? err
              : Promise.reject(err)
          )
      );
  ...
  function extractChangingRecords(to, from) {
    ...
    for (let i = 0; i < len; i++) {
      ...
      if (recordTo) {
        ...
        if (
          !from.matched.find((record) => isSameRouteRecord(record, recordTo))
        ) {
